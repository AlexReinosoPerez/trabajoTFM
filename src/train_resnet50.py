...
LEARNING_RATE = 3e-4
PATIENCE = 5
MODEL_NAME = "models/resnet50_vangogh_2.pth"
...
# Inicializar con pesos preentrenados
...
with mlflow.start_run(run_name=f"train_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_acc > best_acc:
            ...
            torch.save(model.state_dict(), "src/models/resnet50_vangogh_2.pth")
...
# asegúrate de que el mejor modelo está en memoria
os.makedirs("src/models", exist_ok=True)
torch.save(model.state_dict(), "src/models/resnet50_vangogh_2.pth")
